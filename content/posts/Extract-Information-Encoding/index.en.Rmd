...
2. Choose the correct option in the parameter encoding when reading the data. 

*For database*, when you are establishing connection, use the encoding of the table
__`con<-DBI::dbConnect(odbc::odbc(),"Nombre_odbc",encoding="Latin1")`__

If different tables from the database use different encoding, use a different _con_ when using `DBI::dbgetquery(con, glue::glue("...")) `

For excel, good news! 
    `readxl::read.xlsx` identifies the encoding by itself and bring the information correctly. 
...
Last tip for Spanish users: Most common encoding for Spanish language are UTF-8
